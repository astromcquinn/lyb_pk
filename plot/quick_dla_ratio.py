...
labels = [r"$\widehat{P}_{\alpha \alpha}$",
            r"$\widehat{P}_{TT}}$",
            r"$\widehat{P}_{\alpha \beta}$",
            r"$\widehat{\cal{P}}_{\beta \beta}$"]
...
fig.text(0.02, 0.5, r"$kP(k)/\pi$", ha='center', va='center', rotation='vertical',fontsize=25)
plt.grid(False)
...
for j in range(3):
    if zidx<7:
        ...
        ax[j].plot(k_x,np.log10(k*t_keepDLA.paa.values/np.pi),color=colors[0],marker='.',ls="--")
        ax[j].plot(k_x,np.log10(k*t_keepDLA.ptt.values/np.pi),color=colors[1],marker='.',ls="--")
        ax[j].plot(k_x,np.log10(k*t_keepDLA.pab.values/np.pi),color=colors[2],marker='.',ls="--")
        ax[j].plot(k_x,np.log10(k*t_keepDLA.pbb.values/np.pi),color=colors[3],marker='.',ls="--")
        ax[j].plot(k_x,np.log10(k*t_removeDLA.paa.values/np.pi),color=colors[0],marker='.')
        ax[j].plot(k_x,np.log10(k*t_removeDLA.ptt.values/np.pi),color=colors[1],marker='.')
        ax[j].plot(k_x,np.log10(k*t_removeDLA.pab.values/np.pi),color=colors[2],marker='.')
        ax[j].plot(k_x,np.log10(k*t_removeDLA.pbb.values/np.pi),color=colors[3],marker='.')
